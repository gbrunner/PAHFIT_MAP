...
;SL_PAHFITMAP.PRO is a program that uses PAHFIT (created by JD Smith
; and the SINGS Team).  This program recreates lines maps,
;EW maps, uncertainty maps, and more for Spitzer IRS SL observations

;NOTE: PAHFIT must be compiled before running. Call PAHFIT before
;running this program.  i.e '.run pahfit.pro'

;ALSO: Still need to 'CROP' the cube yourself.  I will eventually fix
;this!  

;EXAMPLE
;.run pahfit.pro
;sl_pahfitmap, 'sl_cube.fits', 'sl_unc.fits', 'sl1_wave.tbl', 
;'sl2_wave.tbl', z = 0, smooth = 1

;INPUT:

;sl_cube.fits - A data cube of all of the SL2 + SL1 spectral data

;sl_unc.fits - A data cube of all of the SL2 = SL1 uncertainty data

;sl1_wave.tbl - A table containind the wavelenths for the SL1 data.  
;PAHFIT needs the wavelength to fit spectra.

;sl2_wave.tbl - A table containind the wavelenths for the SL2 data.  
;PAHFIT needs the wavelength to fit spectra.

;z - The redshift of the object

;smooth - 1 for smoothing by the PSF and 2 for smoothing by 3x3 pixel box

pro sl_pahfitmap, sl_cube, sl_unc, sl1_wave_tbl, sl2_wave_tbl, z=redshift, smooth = yes

;Read in cubes
;SL cubes concatonated in SL_CUBE_JOIN.PRO
sl_cube = mrdfits(sl_cube, 0, sl_cube_header)
sl_unc = mrdfits(sl_unc, 0, sl_unc_header)
...
;Whole Cube
;NEED TO MODIFY THIS BEFORE RUNNING!
;THIS CROPS THE DATA CUBES!
sl1_cube = sl_cube[1:45,*,77:193]
sl1_unc  = sl_unc[1:45,*,77:193]
sl2_cube = sl_cube[1:45,*,0:76]
sl2_unc  = sl_unc[1:45,*,0:76]

;sl1_cube = sl_cube[44:45,3:4,77:193]
;sl1_unc  = sl_unc[44:45,3:4,77:193]
;sl2_cube = sl_cube[44:45,3:4,0:76]
;sl2_unc  = sl_unc[44:45,3:4,0:76]

;Read in Wavelength files
readcol, sl1_wave_tbl,  sl1_wave, sl1_junk1, sl1_junk2
readcol, sl2_wave_tbl,  sl2_wave, sl2_junk1, sl2_junk2

;Truncate wavelength just as we did the cubes
sl1_wave = sl1_wave[0:116]
sl2_wave = sl2_wave[0:76]
...
;Cube Match (match cubes before putting in PAHFIT)
;Make Matching the Cubes optional
...
;CUBE SMOOTHING
;Type 1 to smooth by the PSF and 2 smooth by a box

;______________________________________________________________________
;Smoothing by the wavelength dependant PSF
;Assumes FWHM at 5.5 microns is.5
...
psf = 0.5*(wave/wave[0])
;psf = wave/wave[0]
psf_smoothed_cube = sl_cube
psf_smoothed_unc = sl_unc

sl_z = n_elements(wave)-1

for a=0,sl_z,1 do begin
psf_smoothed_cube[*,*,a] = filter_image(sl_cube[*,*,a], FWHM = psf[a], /ALL_PIXELS)
endfor

for a=0,sl_z,1 do begin
psf_smoothed_unc[*,*,a] = filter_image(sl_unc[*,*,a], FWHM = psf[a], /ALL_PIXELS)
endfor

mwrfits, psf_smoothed_cube, 'sl_psf_smooth.fits', sl_cube_header
mwrfits, psf_smoothed_unc, 'sl_psf_smooth_unc.fits', sl_cube_header

sl_cube = psf_smoothed_cube
sl_unc = psf_smoothed_unc

new_cube = psf_smoothed_cube
new_unc = psf_smoothed_unc
...
mwrfits, new_cube, 'sl_box_smooth_cube_test.fits'
mwrfits, new_unc, 'sl_box_smooth_unc_test.fits'
...
fit=pahfit(wave, sl_cube(i,j,*), sl_unc(i,j,*), REDSHIFT=z)
;,/PLOT_PROGRESS, XSIZE = 1000, YSIZE = 600)
...
;WRITE ARRAYS TO FITS FILES WITH UPDATED HEADER INFO
;Header info
;sl_crpix1 = sxpar(sl_cube_header, 'CRPIX1') ;Ref pix x
;sl_crpix2 = sxpar(sl_cube_header, 'CRPIX2') ;Ref pix y
sl_cdelt1 = sxpar(sl_cube_header, 'CDELT1') ;Degree/Pix
sl_cdelt2 = sxpar(sl_cube_header, 'CDELT2') ;Degree/Pix
sl_crval1 = sxpar(sl_cube_header, 'CRVAL1') ;RA
sl_crval2 = sxpar(sl_cube_header, 'CRVAL2') ;Dec

;DO I NEED TO EDIT THE HEADER VALUES?
;IF SO, DO IT HERE!
...
;Updated Cube headers
hist = strarr(3)
hist[0] = 'PAHMAP'
hist[1] = 'This file was produced by running pahfit.pro over the joined SL IRS Spectral Cubes'
hist[2] = 'This is the PAHFIT CUBE'
sxaddhist, hist, sl_cube_header
sxdelpar, sl_cube_header, ['CRPIX1']
sxdelpar, sl_cube_header, ['CRPIX2']
sxdelpar, sl_cube_header, ['CRVAL1']
sxdelpar, sl_cube_header, ['CRVAL2']
sxdelpar, sl_cube_header, ['CDELT1']
sxdelpar, sl_cube_header, ['CDELT2']
sxaddpar, sl_cube_header, 'CRPIX1', sl_crpix1
sxaddpar, sl_cube_header, 'CRPIX2', sl_crpix2
sxaddpar, sl_cube_header, 'CRVAL1', sl_crval1
sxaddpar, sl_cube_header, 'CRVAL2', sl_crval2
sxaddpar, sl_cube_header, 'CDELT1', sl_cdelt1
sxaddpar, sl_cube_header, 'CDELT2', sl_cdelt2

;Update map headers
sl_header = sl_cube_header
map_hist = strarr(4)
map_hist[0] = 'PAHMAP'
