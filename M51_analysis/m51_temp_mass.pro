...
;convert units to flux
;ll_factor = 25.8064/(4.25e10)
;sl_factor = 3.42250/(4.25e10)

;h28_map = h28_map*ll_factor
;h17_map = h17_map*ll_factor
;h12_map = h12_map*sl_factor
;h9_map =  h9_map*sl_factor
;h8_map =  h8_map*sl_factor
;h7_map =  h7_map*sl_factor


H28_smooth = h28_map
;h17_smooth = h17_map
;h28_smooth = filter_image(h28_map, FWHM = 1, /ALL)
;h17_smooth = filter_image(h17_map, FWHM = 1.65, /ALL)
...
h17_smooth = filter_image(h17_map, FWHM = 1.65, /ALL)
h12_smooth = filter_image(h12_map, FWHM = 7, /ALL)
h9_smooth = filter_image(h9_map, FWHM = 8.91, /ALL)
h8_smooth = filter_image(h8_map, FWHM = 10.74, /ALL)
h7_smooth = filter_image(h7_map, FWHM = 12.42, /ALL)
...
h17_unc_smooth = filter_image(h17_unc, FWHM = 1.65, /ALL)
h12_unc_smooth = filter_image(h12_unc, FWHM = 7, /ALL)
h9_unc_smooth = filter_image(h9_unc, FWHM = 8.91, /ALL)
h8_unc_smooth = filter_image(h8_unc, FWHM = 10.74, /ALL)
h7_unc_smooth = filter_image(h7_unc, FWHM = 12.42, /ALL)

mwrfits, h28_smooth, 'H28_smooth_again.fits', h28_header
mwrfits, h17_smooth, 'H17_smooth_again.fits', h17_header
mwrfits, h12_smooth, 'H12_smooth_again.fits', h12_header
mwrfits, h8_smooth, 'H8_smooth_again.fits', h8_header
mwrfits, h9_smooth, 'H9_smooth_again.fits', h9_header
mwrfits, h7_smooth, 'H7_smooth_again.fits', h7_header
...
mwrfits, h12_smooth, 'H12_interpol_again.fits', h28_header
mwrfits, h9_smooth, 'H9_interpol_again.fits', h28_header
mwrfits, h8_smooth, 'H8_interpol_again.fits', h28_header
mwrfits, h7_smooth, 'H7_interpol_again.fits', h28_header
...
;ni_gi(i,j,*) = 4*!pi*s_flux(i,j,*)/(h*A*frequency*g*beam)
delta_ni_gi(i,j,*) = 4*!pi*s_unc(i,j,*)/(h*A*frequency*g)
...
;print warm and cold H2 maps
;mwrfits, t_cold, 'cold_hydrogen_temperature_again.fits', h28_header
;mwrfits, t_warm, 'warm_hydrogen_temperature_again.fits', h28_header

;calculate number density
pf_cold = 0.0247*t_cold[*,*]/(1-exp(-6000/t_cold[*,*]))
;n = ni_gi[*,*,0]*pf/(exp(-energy[0]/t_warm))
n_cold = ni_gi[*,*,0]*pf_cold/(exp(-energy[0]/t_cold[*,*]))

pf_warm = 0.0247*t_warm[*,*]/(1-exp(-6000/t_warm[*,*]))
;n = ni_gi[*,*,0]*pf/(exp(-energy[0]/t_warm))
n_warm = ni_gi[*,*,2]*pf_warm/(exp(-energy[2]/t_warm[*,*]))
...
cold_m_hydrogen = n_cold*8.9421e-20
warm_m_hydrogen = n_warm*8.9421e-20

;n0_para = ni_gi[*,*,0]*pf_cold/(exp(-energy[0]/t_cold[*,*]))
;n1_ortho = ni_gi[*,*,1]*pf_cold/(exp(-energy[1]/t_cold[*,*]))
;n2_para = ni_gi[*,*,2]*pf_warm/(exp(-energy[2]/t_warm[*,*]))
;n3_ortho = ni_gi[*,*,3]*pf_warm/(exp(-energy[3]/t_warm[*,*]))
;n4_para = ni_gi[*,*,4]*pf_warm/(exp(-energy[4]/t_warm[*,*]))
;n5_ortho = ni_gi[*,*,5]*pf_warm/(exp(-energy[5]/t_warm[*,*]))
;cold_mass = n0_para*8.942e-20
;warm_mass = n2_para*8.942e-20

;mwrfits, cold_m_hydrogen, 'cold_mass_dist_again.fits', h28_header
;mwrfits, warm_m_hydrogen, 'warm_mass_dist_again.fits', h28_header
